...
- 計測した時間の長さに応じて刀を鍛造できる
  - 【追記1】鍛造のWeb上での動きの想定は以下です
     1. ユーザーは計測開始時に「どのくらい勉強するか」目標時間を設定する
     2. 設定している時間中はカウントダウンタイマーが動く
     3. 目標時間を達成したら終了をお知らせ<br>ユーザーが計測終了ボタンを押すと、計測した時間に応じてランダムに刀剣の画像と名前が表示される
  - 時間が短ければ短めの刀種、長ければ長めの刀種がランダムに表示される
...
- 計測時間によって刀をランダムで鍛造する機能
  ...
  - MVPリリース時には刀剣乱舞に実装されている117振りを実装する予定
